...
$orderNotes     = trim($input['orderNotes'] ?? '');
$deliveryOption = $input['deliveryOption'] ?? 'Home Delivery';
$paymentMethod  = $input['paymentMethod'] ?? 'Cash on Delivery';

$allowedDelivery = ['Home Delivery', 'Store Pickup'];
$allowedPayment  = ['Cash on Delivery', 'GCash', 'PayPal', 'Maya'];

if ($fullname === '' || $contactNumber === '') {
    json_error('Full name and contact number are required.');
}

if ($deliveryOption === 'Home Delivery' && $address === '') {
    json_error('Delivery address is required for home delivery.');
}

if (!in_array($deliveryOption, $allowedDelivery, true)) {
    json_error('Invalid delivery option.');
}
...
$sessionId = $userId ? null : guest_session_id();
...
$deliveryFee = $deliveryOption === 'Store Pickup' ? 0 : 50;
$total = $subtotal + $deliveryFee;

mysqli_begin_transaction($conn);

try {
    $orderNumber = generate_order_number($conn);

    $stmt = mysqli_prepare(
        $conn,
        "INSERT INTO orders
            (order_number, user_id, fullname, email, contact_number, address,
             order_notes, delivery_option, payment_method, subtotal, delivery_fee, total_amount, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Pending')"
    );
    ...
    // Clear the cart
    if ($userId) {
        $clear = mysqli_prepare($conn, "DELETE FROM cart_items WHERE user_id = ?");
        mysqli_stmt_bind_param($clear, "i", $userId);
    } else {
        $clear = mysqli_prepare($conn, "DELETE FROM cart_items WHERE session_id = ?");
        mysqli_stmt_bind_param($clear, "s", $sessionId);
    }
